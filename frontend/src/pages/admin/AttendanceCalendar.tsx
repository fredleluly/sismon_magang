...
const AttendanceCalendar: React.FC = () => {
  ...
  const handleSaveThreshold = async () => {
    ...
    try {
      const res = await AttendanceAPI.setTodayThreshold(
        modalThresholdTime,
        modalThresholdAlasan,
        targetDate,
      );
      if (res && res.success) {
  ...
  const handleResetThreshold = async () => {
    ...
    try {
      ...
      if (res && res.success) {
        ...
        setThresholdAlasan("");
        showToast(
          "Jam telat dikembalikan ke default (" + defaultThreshold + ")",
          "success",
        );
        setShowThresholdModal(false);
      } else {
  ...
  const handleSaveStatus = async () => {
    ...
    try {
      ...
      const jamMasukToSend =
        needsTime && isValidTime(editJamMasuk) ? editJamMasuk : "-";
      const jamKeluarToSend =
        needsTime && isValidTime(editJamKeluar) ? editJamKeluar : "";
      ...
      if (
        ...
        res = await AttendanceAPI.updateStatus(
          editingAtt._id,
          editStatus,
          needsTime && isValidTime(editJamMasuk) ? editJamMasuk : undefined,
          needsTime && isValidTime(editJamKeluar) ? editJamKeluar : undefined,
        );
      }
      if (res && res.success) {
        ...
        if (savedRecord) {
          ...
          setSelectedDayData((prev) => {
            const withoutOld = prev.filter(
              (a) => a._id !== editedId && a._id !== savedRecord._id,
            );
        ...
        >
          <div style={{ marginBottom: 12 }}>
            ...
            >
              ...
              {selectedDate ? "Tanggal Terpilih" : "Hari Ini"}
            </h3>
          ...
          <div>
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: '6px' }}><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z" /><path d="m15 5 4 4" /></svg> Ubah Jam Telat {selectedDate ? "Tanggal Ini" : "Hari Ini"}
            </button>
          </div>
        </div>
      </div>
              ...
              <div className="form-body">
                ...
                {statusTanpaJam.includes(editStatus) && (
                  ...
                  >
                    <span>ℹ️</span>
                    <span>
                      Status <strong>{editStatus}</strong> tidak memerlukan jam
                      masuk & jam keluar.
                    </span>
                  </div>
                )}
            ...
            >
              ...
              <div style={{ padding: "24px" }}>
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 12, color: "#0369a1", lineHeight: 1.4 }}
                  >
                    Perubahan ini hanya berlaku untuk{" "}
                    <strong>tanggal yang dipilih</strong>. Default jam telat:{" "}
