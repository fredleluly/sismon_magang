...
router.get('/admin', auth, adminOnly, async (req, res) => {
  try {
    ...
    const workDistribution = await WorkLog.aggregate([{ $match: { status: 'Selesai', ...dateFilter } }, { $group: { _id: '$jenis', count: { $sum: 1 } } }, { $sort: { count: -1 } }]);
    ...
    const recentActivity = await WorkLog.find({ status: 'Selesai' }).populate('userId', 'name').sort({ createdAt: -1 }).limit(5);
...
router.get('/user', auth, async (req, res) => {
  try {
    const stats = await WorkLog.aggregate([{ $match: { userId: req.userId, status: 'Selesai' } }, { $group: { _id: null, berkas: { $sum: '$berkas' }, buku: { $sum: '$buku' }, bundle: { $sum: '$bundle' } } }]);
    ...
    // Work distribution
    const workDistribution = await WorkLog.aggregate([{ $match: { userId: req.userId, status: 'Selesai' } }, { $group: { _id: '$jenis', count: { $sum: 1 } } }, { $sort: { count: -1 } }]);

    // Recent activity
    const recentActivity = await WorkLog.find({ userId: req.userId, status: 'Selesai' }).sort({ createdAt: -1 }).limit(10);
